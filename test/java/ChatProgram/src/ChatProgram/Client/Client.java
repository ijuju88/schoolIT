...
public class Client extends JFrame implements ActionListener, KeyListener{
	...
	//패널에 이미지넣기(투명도..)
	private JPanel Longin_img_panel = new JPanel(){
	...
	private void connection(){
		...
		this.setTitle(id+"님환영합니다 *-*");//타이틀바꾸기
	...
	private void InMessage(String str){ //서버에서부터 들어오는 모든 메세지
		...
		//사용자리스트 ... 리스트를 업데이트 할겁니다
		else if(protocol.equals("user_list_update")){
			//list_user.updateUI(); 로 하면 잘안되네요 ?? JList컴포넌트에 아이템추가할렵니다
			list_user.setListData(vector_user_list);
		}
	...
	/*
	 ...
	 Java에서 IP주소를 가져오는 방법 -> InetAddress.getLocalHost().getAddress() 

	하지만 리눅스 서버에서는 127.0.0.1과 같이 loopback 주소가 나오는 경우가 있다고 합니다
	...
	public void keyReleased(KeyEvent e) { //눌렀다 땠을 때
		//System.out.println("KeyEvent key : "+e);
		if(e.getKeyCode() == 10){ //10은 enter키
			send_message("Chatting/"+my_room+"/" + textField_message.getText().trim()); 
			textField_message.setText("");//채팅을보내면 내용을지웁니다
			textField_message.requestFocus();
		}
	}
