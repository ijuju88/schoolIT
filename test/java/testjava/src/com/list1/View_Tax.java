...
public class View_Tax extends JFrame {
	...
	// 화면 구성과 화면 출력
	public View_Tax(String row1, String row2, String row3) {
		...
		JButton jbTax_Bill_Excel1 = new JButton("계산서엑셀");
		jbTax_Bill_Excel1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				new ViewTaxexcelAction();
			}
		});
		...
		// 도장
		String stamp = (String) crstr.get(5);
	...
	private void getData(String row1) {
		...
		try {
			...
			String str = row1;
			String sql = "select * from Tax where tnum='" + str + "'";
	...
	private void gettotData(String row1) {
		try {
			stmt = conn.createStatement();
			String str = row1;
			String sql = "select * from Tax_key where tnum='" + str + "'";

			ResultSet rs = stmt.executeQuery(sql);

			while (rs.next()) {
				totstr = new Vector<Object>();
				totstr.addElement(rs.getString("TTOTAL"));
				totstr.addElement(rs.getString("Tcash"));
				totstr.addElement(rs.getString("Tcheck"));
				totstr.addElement(rs.getString("Tpromissory"));
				totstr.addElement(rs.getString("Tcredit"));
				totstr.addElement(rs.getString("Tpay"));
