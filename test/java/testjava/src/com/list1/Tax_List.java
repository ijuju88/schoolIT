...
public class Tax_List extends JFrame {
	...
	String row1= null, row2= null, row3= null, row4= null, 
			row5= null, row6= null,row10= null, row11= null;

	int row7=0, row8= 0,row9= 0;
	...
	public Tax_List() {
		...
		String colName[] = { "일련번호", "공급자사업번호", "공급받는자사업번호", "날짜", "품목", "규격", "수량", "단가", "공급가액", "비고" };
		...
		UpdateButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				else{new View_Tax(row1, row2, row3);}
			}
		});
		...
		table.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				 ...
				 row10 = (String) model.getValueAt(table.getSelectedRow(), 9);
		...
		DeleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result == JOptionPane.YES_OPTION) {
					try {
						 PreparedStatement  stmt = conn.prepareStatement("delete from Tax where tnum = '" + row1 + "'");
						stmt.executeUpdate();
					...
					try {
						PreparedStatement  stmt = conn.prepareStatement("delete from Tax_key where tnum = '" + row1 + "'");
						stmt.executeUpdate();
	...
	public void list(int daydata, int daydata1) {
		System.out.println(daydata1);
		String sql="select distinct  tk.TNUM, tk.CNO, tk.ANO, t.Tdate,"
				+ "t.Tlist, t.Tstandard,t.Tamount,t.Tunit_price ,t.Tprovision,"
				+ "t.Tect from TAX t,TAX_KEY tk"
				+ " where substr(tk.TNUM, 1,6) >= TO_DATE"
				+ "('"+daydata+"','YYYYMMDD') AND  substr(tk.TNUM, 1,6) "
				+ "<  TO_DATE('"+daydata1+"','YYYYMMDD')+1 and tk.tnum=t.tnum order by tk.TNUM";
		...
		try {
			...
			while (rs.next()) {
				...
				row[0] = rs.getString("TNUM");
				row[1] = rs.getString("CNO");
				row[2] = rs.getString("ANO");
				
				String str = rs.getString("Tdate");
				row[3] =str.toString().substring(0, 10);
				
				row[4] = rs.getString("Tlist");
				row[5] = rs.getString("Tstandard");
				row[6] = rs.getInt("Tamount");
				row[7] = rs.getInt("Tunit_price");
				row[8] = rs.getInt("Tprovision");
				row[9] = rs.getString("Tect");
