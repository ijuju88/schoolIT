...
<!--
	...
	function delOk(rsno){
		window.open('delOk.jsp?rsno='+rsno+'&currentPage='+<%= currentPage %>,'','width=1 height=1');
	}
-->
</script>
...
<table border="1">
	...
	<tr>
		...
		<td>
			...
			<!-- <button onclick="reservationListPro.jsp">엑셀</button> -->
		</td>
	</tr>
	<tr>
...
<table border="1">
	<tr>
		...
		<td class="tableSubject2">입금/삭제</td>
	</tr>
<%
	// 총 수
	...
	<tr>
		...
		<td>
			<% if( rb.getPay_yn().equals("N") ){ %>
				...
				<input type="button" value="삭제" onClick="javascript:if(confirm('증말? 지울거임??'))delOk(<%= rb.getRsno() %>);" />
			<% }else{%>
				<input type="button" value="삭제" onClick="javascript:if(confirm('증말? 지울거임??'))delOk(<%= rb.getRsno() %>);" />
			<%
